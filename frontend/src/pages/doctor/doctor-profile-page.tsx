            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <StatusPill tone="success">
                  <BadgeCheck size={13} aria-hidden="true" /> Clinora verified
                </StatusPill>
              </div>
          ...
          <div className="min-w-[16rem] rounded-2xl border border-white/[0.07] bg-black/10 p-4">
            ...
            <div className="mt-2 flex items-end justify-between gap-4">
              ...
              <span className="text-xs text-[var(--clinora-text-faint)]">
                {profile.readiness.completedItems} of {profile.readiness.totalItems} optional items
              </span>
            </div>
            <div
              className="mt-3 h-2 overflow-hidden rounded-full bg-white/[0.06]"
              aria-label={`${profile.readiness.percent}% professional profile setup complete`}
            >
              <div
                className="h-full rounded-full bg-[var(--clinora-accent-teal)]"
                style={{ width: `${profile.readiness.percent}%` }}
              />
            </div>
            <p className="mt-3 text-xs leading-5 text-[var(--clinora-text-faint)]">
              Your medical approval is already complete. This only measures optional profile and booking setup.
            </p>
          </div>
        </div>
      </AppSurface>
      ...
      {profile.readiness.missingItems.length ? (
        <AppSurface as="section" variant="elevated" padding="compact" aria-labelledby="profile-next-title">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2 id="profile-next-title" className="text-base font-semibold text-white">
                Finish your professional setup
              </h2>
              <p className="mt-1 text-sm text-[var(--clinora-text-muted)]">
                These are presentation and booking preferences, not credential requirements.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {profile.readiness.missingItems.slice(0, 4).map((item) => (
                <Link
                  key={item.key}
                  to={item.destination}
                  className="rounded-full border border-white/[0.08] bg-white/[0.035] px-3 py-1.5 text-xs font-medium text-slate-300 hover:border-[var(--clinora-border-interactive)] hover:text-white"
                >
                  {item.label}
                </Link>
              ))}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)]">
        <AppSurface as="section" aria-labelledby="professional-profile-title">
          ...
          {message ? (
            <p
              role="status"
              className="mt-5 rounded-xl border border-teal-300/15 bg-teal-300/[0.05] px-4 py-3 text-sm text-teal-100"
            >
              {message}
            </p>
          ) : null}
          {saveError ? (
            <p
              role="alert"
              className="mt-5 rounded-xl border border-rose-300/15 bg-rose-300/[0.05] px-4 py-3 text-sm text-rose-200"
            >
              {saveError}
            </p>
          ) : null}
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-semibold text-white">
              ...
              <span className="text-xs font-normal leading-5 text-[var(--clinora-text-faint)]">
                Existing appointments keep their booked duration.
              </span>
            </label>
          </div>
          <label className="mt-5 grid gap-2 text-sm font-semibold text-white">
            ...
            />
            <span className="text-right text-xs font-normal text-[var(--clinora-text-faint)]">
          ...
          <div className="mt-6 flex flex-col gap-3 border-t border-[var(--clinora-border-subtle)] pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p className={`text-sm ${dirty ? 'text-amber-100' : 'text-[var(--clinora-text-faint)]'}`}>
              {dirty ? 'You have unsaved profile changes.' : 'All profile changes saved.'}
            </p>
            <div className="flex gap-2">
              ...
              <Button variant="appPrimary" disabled={!dirty || saving} onClick={() => void save()}>
          ...
          <div className="mt-5 rounded-2xl border border-white/[0.07] bg-[var(--clinora-surface-nested)] p-5">
            ...
            {draft.currentPosition || draft.currentOrganization ? (
              <p className="mt-2 flex items-start gap-2 text-xs leading-5 text-[var(--clinora-text-muted)]">
                <BriefcaseBusiness size={14} className="mt-0.5 shrink-0" aria-hidden="true" />
            ...
            <p className="mt-4 text-sm leading-6 text-[var(--clinora-text-muted)]">
              {draft.professionalBio ||
                'Add a short professional bio to help Patients understand your clinical focus before booking.'}
            </p>
      ...
      <AppSurface as="section" aria-labelledby="verified-credentials-title">
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-2">
          <section aria-labelledby="qualifications-title">
            <h3 id="qualifications-title" className="flex items-center gap-2 text-base font-semibold text-white">
              <GraduationCap size={17} aria-hidden="true" /> Qualifications
            </h3>
            {profile.credentials.qualifications.length ? (
                ...
                {profile.credentials.qualifications.map((qualification) => (
                  <div key={qualification.id} className="py-4">
                    ...
                    <p className="mt-1 text-xs leading-5 text-[var(--clinora-text-muted)]">
                      {qualification.institution} · {qualification.countryCode} · {qualification.completionYear}
                    </p>
                  </div>
                ))}
              </div>
              ...
              <EmptyState
                ...
                title="No structured qualifications available"
                copy="Your approved application does not currently contain structured qualification entries."
              />
            )}
          </section>

          <section aria-labelledby="credential-files-title">
            <h3 id="credential-files-title" className="flex items-center gap-2 text-base font-semibold text-white">
              <FileText size={17} aria-hidden="true" /> Signup documents
            </h3>
            {profile.credentials.documents.length ? (
              ...
              <EmptyState
                ...
                title="No signup documents available"
                copy="No approved supporting document metadata is available for this profile."
              />
            )}
          </section>
        </div>
...
function TimezoneField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return (
    <label className="grid gap-2 text-sm font-semibold text-white">
      ...
      <span className="text-xs font-normal leading-5 text-[var(--clinora-text-faint)]">
        Used for your Doctor workspace and as the default when publishing future availability.
      </span>
    </label>
  );
}
...
function CredentialFact({
  ...
  return (
    <div className="rounded-2xl border border-[var(--clinora-border-subtle)] bg-[var(--clinora-surface-nested)] p-4">
      <div className="flex items-center gap-2 text-[var(--clinora-info-foreground)]">
        {icon}
      ...
      {privateValue && value ? (
        <p className="mt-2 text-xs text-[var(--clinora-text-faint)]">
          Private credential · visible only in your authenticated Doctor profile.
        </p>
      ) : null}
    </div>
  );
    ...
    <div className="py-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-sm font-semibold text-white" title={document.originalFilename}>
        ...
        <div className="flex shrink-0 gap-2">
          <Button variant="appSecondary" disabled={busy !== null} onClick={() => void open()}>
