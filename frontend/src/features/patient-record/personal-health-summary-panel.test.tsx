...
describe('PersonalHealthSummaryPanel briefing', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.spyOn(longitudinalHealthApi, 'providerStatus').mockResolvedValue({
      status: 'CONFIGURED',
  ...
  it('renders the distinct briefing sections and expands exact evidence for an undated verified lab', async () => {
    ...
    for (const heading of [
      ...
      'What Changed',
      'What Looks Stable',
      'What May Deserve Follow-up',
      'Questions for Your Next Visit',
      'What Clinora Cannot Determine Yet',
    ]) {
    ...
    expect(screen.getAllByRole('link', { name: 'Open source report' })[0]).toHaveAttribute(
      'href',
...
function summary(status: PersonalHealthSummary['aiSummary']['status'], empty = false): PersonalHealthSummary {
  const evidence = empty
    ...
    : [
        {
          evidenceId: 'E1',
          measurementCode: 'HBA1C',
          measurementName: 'HbA1c',
          healthAreaCode: 'GLUCOSE',
          healthAreaTitle: 'Glucose regulation',
          displayValue: '6.8%',
          unit: '%',
          suppliedRange: '4.0-5.6',
          status: 'HIGH' as const,
          clinicalDate: null,
          displayDate: '2026-09-14',
          dateReliable: false,
          chronologyEligible: false,
          dateBasis: 'UPLOAD_FALLBACK',
          sourceReportId: 'report-1',
          sourceReportName: 'Verified lab report',
          verificationStatus: 'PATIENT_CONFIRMED',
          sourceType: 'MEDICAL_REPORT' as const,
        },
      ];
  return {
    period: { preset: 'LAST_12_MONTHS', label: 'Last 12 months', from: '2025-09-15', to: '2026-09-15' },
    snapshot: {
      verifiedReportsAvailable: empty ? 0 : 1,
      reliablyDatedReports: 0,
      dateUncertainVerifiedReports: empty ? 0 : 1,
      trackedVerifiedMeasurements: empty ? 0 : 1,
      healthAreasRepresented: empty ? 0 : 1,
      measurementsWithComparableHistory: 0,
      observationsExcludedByValidation: 0,
      measurementsWithIncompatibleHistory: 0,
      coverageFrom: null,
      coverageTo: null,
    },
    healthPicture: empty
      ? 'There is not enough eligible verified health information to describe your current health picture yet.'
      ...
      : [
          {
            ...
            description: 'HbA1c is 6.8% and above its supplied range.',
            evidenceIds: ['E1'],
          },
        ],
    changes: [],
    ...
    followUpItems: empty
      ? []
      : [{ id: 'F-1', text: 'HbA1c of 6.8% may be worth discussing in clinical context.', evidenceIds: ['E1'] }],
    visitQuestions: empty
      ? []
      : [{ id: 'Q-1', text: 'How should my HbA1c of 6.8% be interpreted?', evidenceIds: ['E1'] }],
    limitations: empty
      ? [{ title: 'Not enough verified information', description: 'More verified information is needed.' }]
      : [
          {
            title: 'Clinical dates are missing',
            description: 'The finding can appear in the current health picture but not in chronological trends.',
          },
        ],
    evidence,
    aiSummary: {
      status,
      overallHealthView: status === 'AVAILABLE' ? 'Your current verified record shows HbA1c is 6.8% (high).' : null,
      keyThemes: [],
      stableContext: [],
      followUpItems: [],
      visitQuestions: [],
      limitations: [],
      reason: status === 'AVAILABLE' ? null : status,
      cached: false,
    },
    disclaimer: 'This briefing is not a diagnosis.',
