...
export function PersonalHealthSummaryPanel() {
  ...
  const selectionChanged = useMemo(
    () =>
      Boolean(
        summary &&
        (summary.period.preset !== period ||
          (period === 'CUSTOM' && (summary.period.from !== customFrom || summary.period.to !== customTo))),
      ),
    [customFrom, customTo, period, summary],
  );

  useEffect(() => {
    void longitudinalHealthApi
      .providerStatus()
      .then(setProvider)
  ...
  async function generate() {
    ...
    setError('');
    try {
      setSummary(await longitudinalHealthApi.generateSummary(request(summary !== null)));
    } catch (caught) {
      setError(longitudinalHealthError(caught, 'We could not generate your Personal Health Summary.'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      setError(longitudinalHealthError(caught, 'We could not export the Personal Health Summary PDF.'));
    } finally {
      setPdfLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <AppSurface as="section" variant="hero">
          ...
          action={
            summary ? (
              <Button
                ...
                disabled={pdfLoading || selectionChanged}
                onClick={() => void downloadPdf()}
              >
                {pdfLoading ? (
                  <RefreshCcw size={15} className="animate-spin" aria-hidden="true" />
                ) : (
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-[minmax(220px,.8fr)_minmax(0,1.2fr)_auto] md:items-end">
          <label className="block">
            ...
            <select
              value={period}
              onChange={(event) => setPeriod(event.target.value as HealthSummaryPeriodPreset)}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-[var(--clinora-text-faint)]">
          <span>
            {provider?.configured
              ? `Clinora AI · ${providerModelLabel(provider.model)}`
              : 'Clinora AI explanation is not configured'}
          </span>
          <span aria-hidden="true">•</span>
          <span>Gemini explains verified facts; it does not calculate medical findings.</span>
        </div>
        {customInvalid ? (
          <p className="mt-3 text-xs text-[var(--clinora-warning-foreground)]">Choose a valid start and end date.</p>
        ) : null}
        {selectionChanged ? (
          <p className="mt-3 text-xs text-[var(--clinora-warning-foreground)]">
            The change window has changed. Refresh before exporting.
          </p>
        ) : null}
        {error ? (
          <p role="alert" className="mt-4 text-sm text-[var(--clinora-danger-foreground)]">
            {error}
          </p>
        ) : null}
      </AppSurface>
      ...
      {!loading && !summary ? (
        <AppSurface as="section" variant="standard">
          <EmptyState
            icon={<HeartPulse size={18} aria-hidden="true" />}
            title="Create your health briefing"
            copy="Clinora will use eligible verified findings already in your Health Record. Reports without a clinical date can inform the current picture, but never a trend."
          />
        </AppSurface>
      ) : null}
...
function SummaryDocument({ summary, refreshing }: { summary: PersonalHealthSummary; refreshing: boolean }) {
  ...
  return (
    <AppSurface
      as="article"
      variant="standard"
      aria-label="Generated Personal Health Summary"
    ...
    >
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-[var(--clinora-text-muted)]">
              {periodLabel(summary)} · Refreshed {formatDateTime(summary.generatedAt)}
            </p>
          </div>
      ...
      >
        ...
        {summary.aiSummary.status !== 'AVAILABLE' ? (
          <p className="mt-3 text-xs leading-5 text-[var(--clinora-text-faint)]">
            {aiUnavailableMessage(summary.aiSummary.reason, summary.aiSummary.status)} The evidence-backed briefing
        ...
        {summary.keyThemes.length ? (
          <div className="grid gap-4 lg:grid-cols-2">
            {summary.keyThemes.map((theme) => (
              <div
                key={theme.id}
                className="rounded-[var(--radius-app-card)] border border-[var(--clinora-border-subtle)] bg-[var(--clinora-surface-nested)] p-5"
              >
          ...
          <SectionEmpty>There are not enough eligible verified findings to form health themes yet.</SectionEmpty>
        )}
      </BriefingSection>

      <BriefingSection
        id="changed"
        eyebrow="Reliable chronology only"
        title="What Changed"
        ...
        {summary.changes.length ? (
          <div className="grid gap-3 sm:grid-cols-2">
            {summary.changes.map((change) => (
              <div
                key={change.id}
                className="rounded-[var(--radius-app-compact)] border border-[var(--clinora-border-subtle)] p-4"
              >
                <p className="text-sm font-semibold text-white">{change.title}</p>
                <div className="mt-3 flex items-center gap-3 text-sm font-semibold tabular-nums text-white">
                  <span>{change.fromValue}</span>
                  <ArrowRight size={15} aria-hidden="true" />
                  <span>{change.toValue}</span>
                </div>
                <p className="mt-1 text-xs text-[var(--clinora-text-faint)]">
                  {formatDate(change.fromDate)} to {formatDate(change.toDate)}
                </p>
          ...
          <SectionEmpty>
            There is not enough safely comparable, reliably dated history to show a qualifying change. Undated findings
            are never used here.
          </SectionEmpty>
        )}
      </BriefingSection>
      ...
      <BriefingSection
        id="stable"
        eyebrow="Balanced context"
        title="What Looks Stable"
        icon={<ShieldCheck size={18} aria-hidden="true" />}
      >
        <InsightList
          items={summary.stableContext}
          evidence={evidence}
          empty="No stable or in-range context qualifies yet."
        />
      </BriefingSection>
      <BriefingSection
        id="follow-up"
        eyebrow="Grounded discussion points"
        title="What May Deserve Follow-up"
        icon={<Stethoscope size={18} aria-hidden="true" />}
      >
        <InsightList
          items={summary.followUpItems}
          evidence={evidence}
          empty="No current verified finding is outside a supplied range."
        />
      </BriefingSection>
      <BriefingSection
        id="questions"
        eyebrow="Prepare for care"
        title="Questions for Your Next Visit"
        icon={<HelpCircle size={18} aria-hidden="true" />}
      >
        <InsightList
          items={summary.visitQuestions}
          evidence={evidence}
          numbered
          empty="More verified information is needed to create personalized questions."
        />
      </BriefingSection>

      <BriefingSection
        id="limitations"
        eyebrow="Knowledge boundary"
        title="What Clinora Cannot Determine Yet"
          ...
          <div className="space-y-3">
            {summary.limitations.map((item) => (
              <div key={item.title} className="border-l-2 border-[var(--clinora-warning-foreground)]/60 pl-4">
                ...
                <p className="mt-1 text-xs leading-5 text-[var(--clinora-text-muted)]">{item.description}</p>
              </div>
            ))}
          </div>
        ) : (
          <SectionEmpty>No additional data limitations were identified for this briefing.</SectionEmpty>
        )}
      </BriefingSection>
      ...
      <div className="mt-8 flex flex-col gap-4 border-t border-[var(--clinora-border-subtle)] pt-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          ...
          className="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-[var(--clinora-info-foreground)]"
        >
          View Full Health Record <ExternalLink size={14} aria-hidden="true" />
        </Link>
      </div>
    </AppSurface>
  );
...
function SummarySnapshot({ summary }: { summary: PersonalHealthSummary }) {
  ...
  return (
    <section
      className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6"
      aria-label="Summary evidence snapshot"
    >
      {metrics.map(([label, value]) => (
        <div key={label} className="rounded-[var(--radius-app-compact)] bg-[var(--clinora-surface-nested)] px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.09em] text-[var(--clinora-text-faint)]">
            {label}
          </p>
...
function BriefingSection({
  ...
  return (
    <section
      className={`mt-8 ${featured ? 'rounded-[var(--radius-app-card)] border border-[var(--clinora-border-interactive)] bg-[var(--clinora-surface-hero)] p-5 sm:p-6' : ''}`}
      aria-labelledby={id}
    >
      <div className="mb-4 flex items-start gap-3">
        ...
        <div>
          ...
          <h3 id={id} className="mt-1 text-xl font-semibold text-white">
            {title}
          </h3>
        </div>
      </div>
...
function InsightList({
  ...
  items: SummaryBriefingItem[];
  evidence: Map<string, SummaryEvidence>;
  empty: string;
  numbered?: boolean;
}) {
  if (!items.length) return <SectionEmpty>{empty}</SectionEmpty>;
  return (
    <div className="divide-y divide-[var(--clinora-border-subtle)] border-y border-[var(--clinora-border-subtle)]">
      {items.map((item, index) => (
        <div key={item.id} className="py-4">
          <div className="flex gap-3">
            {numbered ? (
              <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-[var(--clinora-surface-nested)] text-xs font-semibold text-[var(--clinora-info-foreground)]">
                {index + 1}
              </span>
            ) : null}
            <p className="text-sm leading-6 text-[var(--clinora-text-muted)]">{item.text}</p>
          </div>
          <EvidenceDisclosure ids={item.evidenceIds} evidence={evidence} />
        </div>
      ))}
    </div>
  );
...
function EvidenceDisclosure({ ids, evidence }: { ids: string[]; evidence: Map<string, SummaryEvidence> }) {
  ...
  return (
    <details className="group mt-3">
      <summary className="inline-flex cursor-pointer list-none items-center gap-1.5 text-xs font-semibold text-[var(--clinora-info-foreground)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">
      ...
      <div className="mt-3 space-y-2">
        {items.map((item) => (
          <div
            key={item.evidenceId}
            className="rounded-[var(--radius-app-compact)] border border-[var(--clinora-border-subtle)] bg-black/10 px-3 py-2.5"
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-xs font-semibold text-white">{item.measurementName}</p>
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold tabular-nums text-white">{item.displayValue}</span>
                <RangePill status={item.status} />
              </div>
            </div>
            <p className="mt-1 text-[11px] leading-5 text-[var(--clinora-text-faint)]">
              {item.suppliedRange ? `Supplied range ${item.suppliedRange} · ` : ''}
              {item.dateReliable && item.clinicalDate
                ? formatDate(item.clinicalDate)
                : `Clinical date unavailable${item.displayDate ? ` · uploaded ${formatDate(item.displayDate)}` : ''} (not used for chronology)`}{' '}
              · {verificationLabel(item.verificationStatus)} · {item.sourceReportName}
            </p>
            {item.sourceReportId ? (
              <Link
                to={`/patient/reports/${item.sourceReportId}`}
                className="mt-1 inline-flex text-[11px] font-semibold text-[var(--clinora-info-foreground)]"
              >
                Open source report
              </Link>
            ) : null}
...
function RangePill({ status }: { status: HealthRangeStatus }) {
  ...
  return (
    <StatusPill tone={tone} className="min-h-6 px-2 py-0.5 text-[10px]">
      {status === 'IN_RANGE' ? 'In range' : status === 'LOW' ? 'Low' : status === 'HIGH' ? 'High' : 'Reported'}
    </StatusPill>
  );
}

function SectionEmpty({ children }: { children: ReactNode }) {
  return <p className="text-sm leading-6 text-[var(--clinora-text-muted)]">{children}</p>;
}
function DateInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="block">
      <span className="text-xs font-semibold text-[var(--clinora-text-muted)]">{label}</span>
      <input
        type="date"
        value={value}
        max={todayIso()}
        onChange={(event) => onChange(event.target.value)}
        className="mt-2 min-h-11 w-full rounded-[var(--radius-app-compact)] border border-[var(--clinora-border-subtle)] bg-[var(--clinora-surface-nested)] px-3 text-sm text-white outline-none focus:border-[var(--clinora-border-interactive)]"
      />
    </label>
  );
}
function SummarySkeleton() {
  return (
    <AppSurface as="section" variant="standard" aria-label="Generating summary">
...
function periodLabel(summary: PersonalHealthSummary) {
  return summary.period.from && summary.period.to
    ? `${summary.period.label}: ${formatDate(summary.period.from)} – ${formatDate(summary.period.to)}`
...
function providerModelLabel(model: string) {
  return model === 'gemini-2.5-flash' ? 'Gemini 2.5 Flash' : model;
}
function verificationLabel(value: string) {
  return value === 'DOCTOR_VERIFIED'
    ? 'Doctor verified'
    : value === 'PATIENT_CORRECTED'
      ? 'Patient corrected'
      : value === 'PATIENT_CONFIRMED'
        ? 'Patient confirmed'
        : 'Verified';
}
function aiUnavailableMessage(reason: string | null, status: string) {
  if (status === 'INSUFFICIENT_DATA')
    return 'There is not enough eligible verified health information yet for an AI explanation.';
  ...
  if (reason === 'INVALID_RESPONSE' || reason === 'EMPTY_RESPONSE')
    return 'The AI provider returned a response Clinora could not validate safely.';
