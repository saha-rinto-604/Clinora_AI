...
import { doctorNotificationApi, type DoctorNotification } from './doctor-notification-api';
import { connectDoctorNotificationStream } from './doctor-notification-stream';
import { doctorNotificationTarget } from './doctor-notification-target';
...
export function DoctorNotificationBell({ className }: { className?: string }) {
  ...
  useEffect(() => {
    let active = true;
    const reconcile = () => {
      void Promise.allSettled([doctorNotificationApi.unreadCount(), doctorNotificationApi.list({ limit: 5 })]).then(
        ([count, page]) => {
          if (!active) return;
    ...
    reconcile();
    const disconnect = connectDoctorNotificationStream((notification) => {
      setItems((current) => {
  ...
  const openNotification = async (notification: DoctorNotification) => {
    if (!notification.readAt) {
      try {
        await doctorNotificationApi.read(notification.id);
        setUnread((value) => Math.max(0, value - 1));
      } catch {
        // The authorized destination remains useful if read-state persistence is temporarily unavailable.
      }
    }
    setOpen(false);
    navigate(doctorNotificationTarget(notification));
  };
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className={cn(
            'relative grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[var(--clinora-border-subtle)] bg-[var(--clinora-surface-nested)] text-[var(--clinora-text-muted)] transition hover:border-[var(--clinora-border-interactive)] hover:text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[var(--clinora-focus-ring-soft)]',
            className,
          )}
        >
      ...
      <DropdownMenuContent
        ...
        aria-label="Recent Doctor notifications"
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-white">Notifications</p>
            <p className="mt-0.5 text-xs text-[var(--clinora-text-faint)]">Practice and appointment updates</p>
          </div>
        ...
        {items.length ? (
          ...
          <EmptyState
            ...
            copy="New bookings and Patient schedule changes will appear here."
          />
        )}
        <Link
