...
import { formatObservationValue, formatReference, rangeStateLabel } from './patient-report-observation-presentation';
        ...
        >
          <div className="border-b border-white/[0.07] bg-cyan-300/[0.045] p-5 sm:p-6">
            ...
            <h3 className="mt-2 text-2xl font-semibold tracking-[-0.025em] text-white">
              {cluster.displayTitle || cluster.title}
            </h3>
          ...
          <div className="space-y-6 p-5 sm:p-6">
            {cluster.candidates.length ? (
              cluster.candidates.map((candidate, candidateIndex) => (
                ...
                >
                  ...
                  <ClusterEvidence
                    ...
                    evidence={cluster.evidence.filter((item) =>
                      candidate.supportingObservationIds.includes(item.observationId),
                    )}
                  ...
                  <ClusterEvidence
                    ...
                    evidence={cluster.evidence.filter((item) =>
                      candidate.contradictoryObservationIds.includes(item.observationId),
                    )}
                    observationMap={observationMap}
                  />
              ...
              <p className="text-sm leading-6 text-slate-300">
                These findings can be considered together, but the available evidence does not justify a specific
                condition name.
              </p>
            )}
            ...
            <ClusterEvidence
              ...
              evidence={cluster.evidence.filter(
                (item) =>
                  item.role === 'SUPPORTS' &&
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        {grounded.map(({ observation, clinicalRelevance, role }) => (
          <div key={`${observation.id}-${role}`} className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4">
            ...
            {clinicalRelevance ? (
              <p className="mt-3 border-t border-white/[0.06] pt-3 text-sm leading-6 text-slate-300">
                {clinicalRelevance}
              </p>
            ) : null}
          </div>
