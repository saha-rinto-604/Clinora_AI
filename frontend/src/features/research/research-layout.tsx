...
export function ResearchShell({ children }: { children: ReactNode }) {
  ...
  const handleSearchSubmit = (e: FormEvent) => {
    ...
    if (/new|create|start/.test(query)) {
      setSearchFeedback('');
      navigate('/research/projects/new');
    } else if (/project|protocol|study/.test(query)) {
      setSearchFeedback('');
    ...
    <div className="relative isolate min-h-dvh overflow-x-clip bg-[var(--clinora-bg-canvas)] text-white selection:bg-cyan-500/30">
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        {isActive ? (
                          <motion.span
                            layoutId={reducedMotion ? undefined : 'research-active-indicator'}
      ...
      <div className="relative z-10 min-h-dvh text-white lg:pl-[244px]" style={{ colorScheme: 'dark' }}>
        ...
        <header className="sticky top-0 z-30 flex min-h-12 items-center justify-between border-b border-[var(--clinora-border-subtle)] bg-[var(--clinora-bg-canvas)] px-4 py-2.5 lg:hidden">
          <NavLink to="/research" className="flex items-center gap-2.5 font-semibold text-white">
            <ClinoraBrandMark size="sm" />
            Clinora <span className="text-[var(--clinora-info-foreground)]">Research</span>
          </NavLink>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-b border-white/10 bg-slate-950/95 px-4 py-4 space-y-3 animate-in slide-in-from-top-2">
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/10 text-xs">
              <div className="font-semibold text-slate-200">{displayName}</div>
              <div className="text-[11px] text-slate-400 font-mono">{displayEmail}</div>
            </div>
        ...
        {/* Desktop Top Utility Bar (Search, Status, Live Clock) Matching Patient Portal */}
          ...
          <div className="relative w-full max-w-[480px]">
            ...
            >
              ...
              <input
                ...
                placeholder="Search projects, datasets, cohorts…"
