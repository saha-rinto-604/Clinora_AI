...
7. COMPACT OUTPUT: at most 6 evidence entries per cluster, one short relevance sentence each; 1-2 concise sentences for interpretation/rationale; 0-2 case-specific missing-information items and 0-2 reasonable alternatives per candidate. Use [] instead of filler when no case-specific item is justified. Missing items should say what evidence would distinguish the candidate; alternatives should be plausible from the supplied pattern. Never invent generic filler, symptoms, history, or medication use. Cluster missingEvidence/alternatives can be empty. Do not repeat prose. Use clusters: [] if no meaningful abnormal pattern.
