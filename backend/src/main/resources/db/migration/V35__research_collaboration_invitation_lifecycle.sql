...
CREATE TABLE IF NOT EXISTS research_project_invitations (
    ...
    CONSTRAINT ck_invitations_status CHECK (status IN ('PENDING', 'ACCEPTED', 'DECLINED', 'REVOKED', 'EXPIRED'))
);
...
CREATE INDEX IF NOT EXISTS idx_invitations_project  ON research_project_invitations(project_id);
CREATE INDEX IF NOT EXISTS idx_invitations_invitee  ON research_project_invitations(invitee_user_id);
CREATE INDEX IF NOT EXISTS idx_invitations_status   ON research_project_invitations(status);
CREATE INDEX IF NOT EXISTS idx_invitations_expires  ON research_project_invitations(expires_at) WHERE status = 'PENDING';
